...
import { type Moment, usePersistentMoments } from "~~/hooks/kairos/usePersistentMoments";
import { usePersistentNickname } from "~~/hooks/kairos/usePersistentNickname";
import { provider as alchemyProvider } from "~~/services/web3/alchemyProvider";
...
export function KairosApp() {
  ...
  const handleSave = async () => {
    ...
    try {
      // 1. Create a "Signer" from the user's embedded Privy wallet.
      const ethereumProvider = await embeddedWallet.getEthereumProvider();
      const ethersProvider = new BrowserProvider(ethereumProvider);
      const signer = await ethersProvider.getSigner();

      // 2. Connect the signer to the Alchemy Provider for gasless transactions.
      // Temporär verwenden wir den Standard-Signer bis der Alchemy Provider implementiert ist
      const smartAccountProvider = alchemyProvider.withSigner(signer);

      console.log("Smart Account Address:", await smartAccountProvider.getAddress());
      ...
      // 4. Create an instance of your Kairos contract to be called by the Smart Account.
      // Die Chain-ID dynamisch vom verbundenen Wallet des Benutzers abrufen.
      const chainId = embeddedWallet.chainId === sepolia.id.toString() ? sepolia.id : 31337;
      const kairosContractAddress = deployedContracts[chainId].Kairos.address;
      const kairosContractAbi = deployedContracts[chainId].Kairos.abi;
      const kairosContract = new Contract(kairosContractAddress, kairosContractAbi, smartAccountProvider);

      // 5. Send the transaction through the Smart Account. The user will NOT see a signature pop-up here.
      // The Alchemy provider handles gas sponsorship and bundling.
  ...
  return (
    <main className="min-h-screen w-full bg-gray-900 flex flex-col items-center justify-between p-4 text-white">
      <div
        className={`w-full max-w-sm mx-auto flex-grow flex flex-col ${
          step === "gallery" ? "justify-start pt-6" : "justify-center"
        }`}
      >
        {step === "nickname" && (
        ...
        {step === "capture" && (
          <div className="text-center animate-fade-in flex flex-col items-center justify-center h-full">
            ...
            >
              <svg
                className="h-20 w-20 text-white opacity-90 group-hover:opacity-100 transition-opacity"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <div className="flex-grow p-6 pt-2 overflow-y-auto">
                {moments.map((moment, index) => (
                  <div key={index} className="bg-gray-700 rounded-lg overflow-hidden shadow-lg h-fit">
                    <img
                      src={`https://${gatewayUrl}/ipfs/${moment.imageCid}`}
                      alt={`Moment ${index}`}
                      className="w-full h-32 object-cover"
                    />

                    <div className="p-3">
                      <p className="text-white text-sm font-medium truncate" title={moment.note || "No note"}>
                        {moment.note || "No note"}
                      </p>
      ...
      {/* Navigation */}
      <div className="w-full max-w-sm mx-auto flex justify-center space-x-4 p-4">
        <button
